...
void ros_template_programs::PluginNodeletTopicPublisher::onInit() {
    ...
    timer_ = nh_.createTimer(ros::Duration(sleep_time), &PluginNodeletTopicPublisher::callbackTimer, this);
    // タイマーを生成
...
void ros_template_programs::PluginNodeletTopicPublisher::callbackTimer(const ros::TimerEvent&) {
    pub_.publish(msg_);
